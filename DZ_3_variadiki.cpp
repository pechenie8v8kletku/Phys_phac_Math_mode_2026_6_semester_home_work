...
using namespace std;
// Просли реализовать Tuple через вариадики, тут надо будет поебаться,насколько помню меня попросили реализовать tie_ чтобы оно работало через tie_(...)= реализовано через оператор присваивания+ просил вроде конкатенацию реализвоать, с точки зрения понимания того что происходит
// Я бы сказал самое сложное дз по понимания и на самом деле прикольноен
