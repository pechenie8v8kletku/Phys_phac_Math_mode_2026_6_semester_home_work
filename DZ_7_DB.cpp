...
#include <map>
#include <numeric>
// Ебунявая задача не понял условия на сайте пришлось переделывать на месте оч сильно, мб кому то поможет понять условия.
// Надо было реализвоать че та на подобии SQL запросов в базу данных реализовано через jopу в целом не хватило времени переделать дямбда функции чтобы все таки считывать это из консоли
// вообще возможно стоит сюда вернуться и попробовать прописать полностью нормальную бд.
// Я бы обратил внимание на реализацию select, where,group by на большее времени не хватило сорянчик
struct Student {
